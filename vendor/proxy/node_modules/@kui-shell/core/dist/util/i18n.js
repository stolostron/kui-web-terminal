...
var _default = (plugin, namespace = 'resources') => {
  try {
    ...
    const i18n = locale => {
      try {
        ...
        try {
          return locale && require(`@kui-shell/${plugin}/i18n/${namespace}_${locale.replace(/-.*$/, '')}.json`) || defaultStrings;
        } catch (err) {
    ...
    return function (key, ...parameters) {
      ...
      if (!parameters) return str;else return parameters.map(_ => (_ || '').toString()) // only needed due to https://github.com/microsoft/TypeScript/issues/7014
