...
const debug = (0, _debug.default)('plugin-kubectl/controller/watch/watcher');

function preprocessTable(raw, nCols) {
  ...
  if (lastFullRowIdx < 0) {
    ...
    return {
      leftover: undefined,
      rows: rows.map(line => line.map(value => ({
        key: value,
        value
      }))).filter(_ => _.length > 0)
    };
    ...
    return {
      ...
      rows: rows.slice(0, lastFullRowIdx + 1).map(line => line.map(value => ({
        key: value,
        value
      }))).filter(_ => _.length > 0)
    };
...
class KubectlWatcher {
  ...
  constructor(args, output = (0, _options.formatOf)(args)) {
    this.args = args;
    this.output = output;
    /**
  ...
  getRowsForNamespace(apiVersion, kind, namespace, rowNames) {
    const getCommand = `${(0, _util.getCommandFromArgs)(this.args)} get ${(0, _fqn.kindPart)(apiVersion, kind)} ${rowNames.join(' ')} -n ${namespace} ${this.output ? `-o ${this.output}` : ''}`;
  ...
  getRowsForUser(rows) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const tables = (yield Promise.all(Object.keys(groups).map(namespace => {
      ...
      return {
        ...
        body: (0, _core.flatten)(tables.map(_ => _.body))
      };
    });
  }
  /**
  ...
  onPTYInit(ptyJob) {
    ...
    return _ => __awaiter(this, void 0, void 0, function* () {
      if (typeof _ === 'string') {
        ...
        const rawData = this.leftover ? this.leftover + _ : _; // as a failsafe, since we just read this.leftover, clear it
        // out (read-once); we will then reestablish any residual
        // leftover immediately below

        this.leftover = undefined; // here is where we turn the raw data into tabular data

        const preprocessed = preprocessTable(rawData, this.nCols);
        this.leftover = preprocessed.leftover === '\n' ? undefined : preprocessed.leftover;
        const {
          rows
        } = preprocessed; // now process the full rows into table view updates

        const table = yield this.getRowsForUser(rows);
  ...
  init(pusher) {
    return __awaiter(this, void 0, void 0, function* () {
      this.pusher = pusher; // here, we initiate a kubectl watch, using a schema of our
      // choosing; we ask the PTY to stream output back to us, by using
      // the `onInit` API

      const command = this.args.command.replace(/^k(\s)/, 'kubectl$1').replace(/--watch=true|-w=true|--watch-only=true|--watch|-w|--watch-only/g, '--watch') // force --watch
      .replace(new RegExp(`(-o|--output)(\\s+|=)${this.output}`), '') + ` -o jsonpath='{.metadata.name}{"|"}{.kind}{"|"}{.apiVersion}{"|"}{.metadata.namespace}{"|\\n"}'`; // ^^^^^ keep these in sync with nCols above !!
...
function doGetWatchTable(args) {
  return __awaiter(this, void 0, void 0, function* () {
    try {
      ...
      // command line
      const cmd = args.command.replace(/^k(\s)/, 'kubectl$1').replace(/--watch=true|-w=true|--watch-only=true|--watch|-w|--watch-only/g, ''); // strip --watch
      ...
      if (typeof initialTable === 'string') {
        ...
        return {
          ...
          breadcrumbs: initialTable.breadcrumbs || (yield (0, _formatTable.getNamespaceBreadcrumbs)(initialTable.title, args)),
          title: initialTable.title || (yield (0, _explain.getKind)((0, _util.getCommandFromArgs)(args), args, args.argvNoOptions[args.argvNoOptions.indexOf('get') + 1])),
