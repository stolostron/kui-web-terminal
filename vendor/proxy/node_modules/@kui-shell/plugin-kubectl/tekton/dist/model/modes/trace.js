...
exports.default = exports.traceView = exports.render = void 0;
...
var _success = _interopRequireDefault(require("../../lib/success"));
...
const debug = (0, _debug.default)('plugins/tekton/models/modes/trace');
/**
 * Render a trace view in the given container
 *
 */

const render = (tab, activations, container, opts = {}) => {
  const {
    noCrop = false,
    showStart = false,
    showTimeline = true
  } = opts;
  debug('trace', activations); // add a legned
  ...
  container.appendChild(logTable); // duration of the activation. this will be helpful for
  ...
  activations.forEach((activation, idx) => {
    ...
    : activation.statusCode !== undefined ? activation.statusCode === 0 : activation.response && activation.response.success; // row dom
    ...
    const nextCell = () => line.insertCell(-1); // column 1: activationId cell
    ...
    id.setAttribute('data-activation-id', id.innerText); // clicky.onclick = pip(show(activation))
    // column 2: name cell
    ...
    name.appendChild(nameClick); // command to be executed when clicking on the entity name cell
    ...
    const waitTime = 0;
    const initTime = 0; // column 5|6|7: bar chart cell

    if (showTimeline) {
      const timeline = nextCell();
      (0, _core.empty)(timeline);
      const isRootBar = idx === 0;
      timeline.className = 'log-field log-line-bar-field'; // execution time bar
      ...
      const width = right - left; // on which side should we render the tooltip?

      const balloonPos = right > 0.9 ? 'left' : 'right';
      bar.style.left = 100 * left + '%';
      bar.style.width = 100 * width + '%'; // bar.onclick = pip(show(activation))
      ...
      bar.onmouseout = () => legend.removeAttribute('data-hover-type'); // container initialization bar
      ...
      if (initTime > 0 && !isRootBar) {
        ...
        initTimeBar.onmouseout = () => legend.removeAttribute('data-hover-type'); // activation can fail at init time - if that's the case, initTime === duration
      ...
      if (waitTime > 0 && !isRootBar) {
        ...
        waitTimeBar.classList.add('is-waitTime'); // waitTimeBar.onclick = pip(show(activation))
    ...
    if (showStart) {
      ...
      const time = (0, _core.prettyPrintTime)(activation.start - waitTime, 'short', previousStart);
...
function makeTaskRunsActivationLike(run, pipeline, jsons) {
  ...
  const {
    taskRefName2Task
  } = makeSymbolTables(pipeline, jsons);
...
const traceView = (tab, run, pipeline, jsons) => {
