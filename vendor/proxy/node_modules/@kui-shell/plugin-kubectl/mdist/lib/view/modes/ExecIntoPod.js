...
import { i18n, getPrimaryTabId, eventChannelUnsafe } from '@kui-shell/core';
import { inDebugMode, Loading } from '@kui-shell/plugin-client-common';
import { Terminal as XTerminal } from 'xterm';
import { FitAddon } from 'xterm-addon-fit';
import { getCommandFromArgs } from '../../util/util';
import { isPod } from '../../model/resource';
import { getContainer } from '../../../controller/kubectl/options';
import { ContainerComponent, HYSTERESIS } from './ContainerCommon';
...
export class Terminal extends ContainerComponent {
    constructor(props) {
        super(props);
        this.cleaners = [];
    ...
    ptyCommand() {
        ...
        const command = (args.argsForMode && args.argsForMode.command) ||
            `${getCommandFromArgs(this.props.args)} exec -it ${pod.metadata.name} -c ${container} -n ${pod.metadata.namespace} -- sh`;
    ...
    initStream() {
        const { tab: { REPL: repl } } = this.props;
    ...
    static initTerminal(dom) {
        ...
        const rendererType = inDebugMode() || process.env.RUNNING_SHELL_TEST || process.env.RUNNING_KUI_TEST ? 'dom' : 'canvas';
    ...
    render() {
        if (this.state.dom && !this.state.xterm) {
            ...
            return (React.createElement("div", { className: "kui--full-height kui--terminal kui--relative-positioning", "data-needs-hysteresis": this.needsHysteresis(), "data-got-some-data": this.state.gotSomeData, ref: dom => this.setState({ dom }) }, this.maybeNothingToShow()));
        }
    }
}
/**
    ...
    return __awaiter(this, void 0, void 0, function* () {
        return {
            react: function TerminalProvider(toolbarController) {
                return React.createElement(Terminal, { tab: tab, pod: pod, args: args, toolbarController: toolbarController });
            }
        };
    });
}
...
export const terminalMode = {
    when: (resource) => isPod(resource) && !resource.isSimulacrum,
    mode: {
        mode: 'terminal',
        label: 'Terminal',
        content
...
//# sourceMappingURL=ExecIntoPod.js.map