...
const debug = (0, _debug.default)('plugins/wskflow/subtext');
/**
 * Render deployment status of graph nodes
 *
 */

function _default(tab, actions, activations, graphData, options) {
      ...
      if (actions) {
        ...
        const graphChildrenStatus = (childrens, id, deployed) => {
          return childrens.forEach(children => {
            if (children.id === id) children.deployed = deployed;else if (children.children) return graphChildrenStatus(children.children, id, deployed);
          });
        };
        ...
        if (notDeployed.length > 0 && !activations) {
          if (!options || !options.noHeader) {
            ...
            message.appendChild(examples); // container.appendChild(message)
