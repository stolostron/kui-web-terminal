...
const debug = (0, _debug.default)('plugins/wskflow/graph2doms');
...
function graph2doms(tab, JSONgraph, ifReuseContainer, activations, {
  layoutOptions = {},
  composites = {
    label: {
      fontSize: '4px',
  ...
  return __awaiter(this, void 0, void 0, function* () {
    const [d3, {
      ...
      default: ELK
    }] = yield Promise.all([Promise.resolve().then(() => require('d3')), Promise.resolve().then(() => require('jquery')), Promise.resolve().then(() => require('elkjs/lib/elk.bundled.js'))]);
    ...
    const ssvg = d3.select($(wskflowContainer)[0]).append('svg').attr('id', 'wskflowSVG').attr('data-is-session-flow', !!activations).call(zoom);
    ...
    const defs = svg.append('svg:defs'); // a pattern mask for "not deployed"

    defs.append('svg:pattern').attr('id', 'pattern-stripe').attr('width', 1.2).attr('height', 1.2).attr('patternUnits', 'userSpaceOnUse').attr('patternTransform', 'rotate(45)').append('svg:rect').attr('width', 0.6).attr('height', 2).attr('transform', 'translate(0,0)').attr('fill', '#aaa'); // this will be the opacity of the mask, from #fff (full masking) to #000 (no masking)

    defs.append('svg:mask').attr('id', 'mask-stripe').append('svg:rect').attr('x', 0).attr('y', 0).attr('width', '100%').attr('height', '100%').attr('fill', 'url(#pattern-stripe)'); // a heavier pattern mask

    defs.append('svg:pattern').attr('id', 'pattern-stripe-heavy').attr('width', 1.2).attr('height', 1.2).attr('patternUnits', 'userSpaceOnUse').attr('patternTransform', 'rotate(45)').append('svg:rect').attr('width', 1).attr('height', 2).attr('transform', 'translate(0,0)').attr('fill', '#aaa'); // this will be the opacity of the mask, from #fff (full masking) to #000 (no masking)

    defs.append('svg:mask').attr('id', 'mask-stripe-heavy').append('svg:rect').attr('x', 0).attr('y', 0).attr('width', '100%').attr('height', '100%').attr('fill', 'url(#pattern-stripe-heavy)'); // define an arrow head

    const arrowHead = id => {
      const marker = defs.append('svg:marker').attr('id', id).attr('viewBox', '0 -5 10 10').attr('markerUnits', 'userSpaceOnUse').attr('markerWidth', 4.2) // marker settings
      .attr('markerHeight', 7).attr('orient', 'auto');
    ...
    function drawGraph(nodes, links) {
      debug('drawGraph'); // #1 add the nodes' groups

      const nodeData = root.selectAll('.node').data(nodes, function (d) {
        return d.id;
      });
      const node = nodeData.enter().append('g').attr('transform', function (d) {
      ...
      .attr('data-name', function (d) {
        ...
        const label = d.type === 'Entry' || d.type === 'Exit' ? d.type : d.multiLineLabel ? d.fullFunctionCode.replace(/\n/g, '') : d.label;
      ...
      node.append(d => {
        ...
        if (activations) {
          ...
          if (d.children) {
            if (d.type === 'try') {
              ...
              if (label.indexOf(':') !== -1) {
                qtipText = "<span class='qtip-prefix'>Try-Catch</span> <span style='color: orange'>" + label.substring(label.indexOf(':') + 1) + '</span>'; // $(this).siblings("use").attr("xlink:href", "#retryIconOrange").attr("href", "#retryIconOrange");
              } else if (label.indexOf('Repeat') !== -1) {
                const num = label.split(' ')[1];
                qtipText = "<span class='qtip-prefix'>Repeat </span>" + "<span style='color: orange'>" + num + ' times</span> then continue'; // $(this).siblings("use").attr("xlink:href", "#retryIconOrange").attr("href", "#retryIconOrange");
              } else {
          ...
          if (d.properties && d.properties.choice) {
            qtipText += "<div class='top-pad'>This is the <span style='color: orange;'>Y</span>/<span style='color: #DC7633;'>N</span> condition of an if</div>"; // also highlight the edges
        ...
        if (qtipText && qtipText.length !== 0) {
          ...
          if (qtipPre) $('#qtip').addClass('qtip-pre');else $('#qtip').removeClass('qtip-pre');
        ...
        if (d.onclick) {
          ...
          if (d.visited) {
            ...
            if (d.type === 'Exit') {// console.log(fsm.States[d.id].act[0]);
              ...
              if (d.visited.length === 1) {
                ...
                }).click(function () {// repl.exec(`wsk action get "${d.name}"`, {sidecarPrevious: 'get myApp', echo: true});
      ...
      const textElements = node.append('text').attr('width', d => d.width).attr('x', function (d) {
        ...
        if (d.children) {
          ...
          return d.height / 2 + (d.type === 'Entry' || d.type === 'Exit' ? 1.5 : d.type === 'Dummy' ? 1.5 : d.type === 'let' ? 3.5 : 2);
        }
      ...
      }).style('pointer-events', 'none').attr('multi-line-label', d => d.multiLineLabel && d.multiLineLabel.join('\n')).text(function (d) {
        if (d.type === 'retain' || d.type === 'Dummy' && d.label === d.id) {
          ...
          return 'Catch'; // else if(d.type == "try_catch" || d.type === 'retry'){
        } else if (d.children) {
          // return "Try Catch";
          return d.label;
          /* else if(d.type == "if"){
          ...
          if (typeof d.value === 'string') {
            ...
            const array = d.value.slice(0, maxLen - 1).map(() => _).concat(rest);
      ...
      for (let idx = 0; idx < textElements[0].length; idx++) {
        ...
        if (label) {
          const {
            maxLineLength
          } = (0, _util.textualPropertiesOfCode)(label.value);
          const tabWidth = 2;
          ...
          label.value.split(/\n/).forEach(line => {
            ...
            d3.select(textElement).append('tspan').text(line.replace(ws, '')).attr('x', farLeft + initialWhitespace * tabWidth).attr('dy', '1.25em');
          });
        }
      } // #2 add paths with arrows for the edges


      root.selectAll('.link').data(links, function (d) {
        return d.id;
      }).enter().append('path').attr('id', function (d) {
        return d.id;
      }).attr('d', 'M0 0').attr('marker-end', function (d) {
        if (d.visited) {
          return 'url(#greenEnd)';
      ...
      const addEdgeLabels = () => links.forEach(edge => {
        if (edge.labels) {
          edge.labels.forEach(({
            ...
            d3.select('#' + edge.source).append('text').classed('edge-label', true).attr('x', 10).attr('y', 0).text(text);
          });
        }
        ...
        if (edge.sourcePort && (edge.sourcePort.indexOf('_ptrue') !== -1 || edge.sourcePort.indexOf('_pfalse') !== -1)) {
          ...
          if (t !== undefined && d !== undefined) {
            ...
            d3.select('#' + edge.source).append('text').classed('edge-label', true).attr('x', x).attr('y', y).text(t).classed('edge-was-traversed', thisEdgeWasTraversed).classed(cssClass, true);
          }
        }
      });

      setTimeout(() => {
        d3.select('.repeat').append('use').attr('xlink:href', '#retryIconNormal').attr('href', '#retryIconNormal').attr('x', 10).attr('y', -14);
      }, 0);
    ...
    const doneRendering = elk.layout(JSONgraph, {
      ...
      elkData = data; // By default, the graph resizes to fit the size
      ...
      const getNodes = function (graph) {
        const queue = [graph];
        const nodes = [];
        let parent; // note that svg z-index is document order, literally
    ...
    const notify = () => handlers.forEach(_ => _({
      applyAutoScale,
      customZoom
    }));
    /**
    ...
    $(window).unbind('resize').resize(() => {
      if (customZoom && $('#wskflowSVG').attr('viewBox') !== undefined) {
        ...
        $('#wskflowSVG').removeAttr('viewBox'); // adjust transform to let the graph be in the same size and location
