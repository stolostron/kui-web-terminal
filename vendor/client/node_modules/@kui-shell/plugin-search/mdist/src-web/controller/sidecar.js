...
exports.buildSidecar = function (type, data, resource) {
    ...
    if (type !== 'query') {
        ...
        if (lodash.get(data, 'items[0].kind', '') === 'pod') {
            modes.push(logging_1.logTab(data.items[0]));
        }
        ...
        if (!lodash.get(resource, 'errors', '') && lodash.get(data, 'getResource', '') === '') {
            modes.push(yaml_1.yamlTab(resource));
        }
    }
    ...
    if (lodash.get(data, 'related', '').length > 0 && type !== 'query') {
        modes.push(related_1.relatedTab(data, type));
    }
    ...
    return {
        ...
        summary: type !== 'query' ? summary_1.buildSummary(data.items[0]) : related_1.buildRelated(data.related, type),
        metadata: {
                ...
                if (search_1.isSearchAvailable()) {
                    HTTPClient_1["default"]('post', 'search', search_queries_1.SEARCH_RELATED_QUERY(userQuery.keywords, userQuery.filters))
                        ...
                        if (!data || data.items.length === 0) {
                            ...
                            HTTPClient_1["default"]('post', 'console', search_queries_1.SEARCH_ACM_QUERY(data.items[0]))
                                .then(function (resp) {
