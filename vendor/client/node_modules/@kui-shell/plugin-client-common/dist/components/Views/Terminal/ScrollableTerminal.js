...
class ScrollableTerminal extends React.PureComponent {
  ...
  allocateUUIDForScrollback() {
    if (this.props.config.splitTerminals && !(0, _core.isPopup)()) {
  ...
  scrollback(pinBlock, capturedValue, sbuuid = this.allocateUUIDForScrollback()) {
    const state = {
      uuid: sbuuid,
      cleaners: [],
  ...
  shouldPinTheBlock(event) {
    return this.props.config.splitTerminals && this.props.config.enableWatcherAutoPin && !(0, _core.isPopup)() && (0, _core.isWatchable)(event.response) && event.evaluatorOptions.alwaysViewIn !== 'Terminal' && event.execOptions.alwaysViewIn !== 'Terminal';
  }
