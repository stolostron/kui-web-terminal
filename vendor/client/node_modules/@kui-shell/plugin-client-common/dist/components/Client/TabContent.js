...
var _Loading = _interopRequireDefault(require("../spi/Loading"));
...
class TabContent extends React.PureComponent {
  ...
  defaultLoading() {
    return React.createElement(_Loading.default, {
      description: strings('Please wait while we connect to your cloud')
    });
  }

  sessionInitDoneMessage() {
    return this.state.showSessionInitDone && this.state.sidecarWidth === "0%"
    /* Closed */
    && React.createElement(_context.default.Consumer, null, config => config.loadingDone && React.createElement("div", {
      className: "kui--repl-message kui--session-init-done"
    }, React.createElement("span", {
      className: "repl-block"
    }, config.loadingDone(this.state.tab.REPL))));
  }
  ...
  terminal() {
    if (this.state.sessionInit !== 'Done') {
      ...
      return React.createElement(React.Fragment, null, this.sessionInitDoneMessage(), React.createElement(_context.default.Consumer, null, config => React.createElement(_ScrollableTerminal.default, Object.assign({}, this.props, {
        tab: this.state.tab,
        config: config,
