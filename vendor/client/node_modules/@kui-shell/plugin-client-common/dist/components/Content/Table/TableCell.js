...
function renderCell(kuiRow, justUpdated, tab, repl) {
  return function KuiTableCell(cell, cidx) {
    ...
    const outerClassName = 'cell-inner ' + (cidx === 0 ? (kuiRow.css || '') + (kuiRow.onclick ? ' clickable' : '') : (kuiRow.attributes[cidx - 1].css || '') + (kuiRow.attributes[cidx - 1].onclick ? ' clickable' : '')); // the text value of the cell
    ...
    return React.createElement(_carbonComponentsReact.TableCell, {
      ...
      key: kuiRow.attributes[cidx - 1].css
