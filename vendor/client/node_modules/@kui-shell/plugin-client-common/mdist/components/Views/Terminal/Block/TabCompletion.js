...
import { getCurrentTab, findCompletions as findCompletionsFromRegistrar, _split } from '@kui-shell/core';
...
function shellescapeIfNeeded(str, prefix) {
...
export class TabCompletionState {
            ...
            if (toBeCompletedIdx >= 0 || completingTrailingEmpty) {
                ...
                return new Promise(resolve => {
                    ...
                    const myEnumeratorAsync = global.setTimeout(() => __awaiter(this, void 0, void 0, function* () {
                        const completions = yield findCompletionsFromRegistrar(input.props.tab || getCurrentTab(), commandLine, spec);
    ...
    update(spec) {
        ...
        this.input.setState({
            tabCompletion: !completions || completions.length === 0
                ? undefined
...
class TabCompletionStateWithSingleSuggestion extends TabCompletionState {
    ...
    render() {
        ...
        const extra = typeof this.completion === 'string'
...
class TabCompletionStateWithMultipleSuggestions extends TabCompletionState {
    ...
    renderOneCompletion(completion, idx) {
        ...
        return (React.createElement("div", { className: "kui--tab-completions--option", key: idx, "data-value": value },
            React.createElement(Button, { href: "#", size: "small", tabIndex: 1, onClick: () => this.completeWith(idx) },
                React.createElement(React.Fragment, null,
                    React.createElement("span", { className: "kui--tab-completions--option-partial" }, preText),
                    React.createElement("span", { className: "kui--tab-completions--option-completion" }, postText)))));
    }
    /** Helper for `willUpdate` */
    eq(c1, c2) {
        return ((typeof c1 === 'string' && typeof c2 === 'string' && c1 === c2) ||
    ...
    willUpdate(completions) {
        return !(this.completions.completions.length === completions.completions.length &&
    ...
    estimateGridColumnWidth() {
        const longest = this.completions.completions
            .map(completion => typeof completion === 'string'
    ...
    render() {
        ...
        return (React.createElement("div", { className: "kui--tab-completions grid-layout", style: { gridTemplateColumns: `repeat(auto-fill, minmax(calc(${ex}ex + ${em}em), auto))` } }, this.completions.completions.map((_, idx) => this.renderOneCompletion(_, idx))));
    }
}
/**
 ...
 *
 */
export default function startTabCompletion(input, evt) {
