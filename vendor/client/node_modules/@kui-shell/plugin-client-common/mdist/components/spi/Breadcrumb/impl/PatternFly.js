...
export default class PatternflyBreadcrumb extends React.PureComponent {
    render() {
        const currentPageIdx = getCurrentPageIdx(this.props);
        return (React.createElement(Breadcrumb, null, this.props.breadcrumbs.map((_, idx) => {
            const onClick = _.command && (() => this.props.repl.pexec(_.command));
            return (React.createElement(BreadcrumbItem, { to: onClick ? '#' : undefined, key: idx, className: [
                    _.className,
                    _.deemphasize && 'kui--secondary-breadcrumb',
                    idx === currentPageIdx && 'kui--current-page-breadcrumb'
                ]
                    .filter(_ => _)
                    .join(' '), isActive: idx === this.props.breadcrumbs.length - 1, onClick: onClick }, _.label));
        })));
    }
}
