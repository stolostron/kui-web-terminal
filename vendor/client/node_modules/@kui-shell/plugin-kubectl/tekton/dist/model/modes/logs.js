...
const mode = {
  ...
  content: (tab, run) => __awaiter(void 0, void 0, void 0, function* () {
    const [{
      ...
      items: pods
    }] = yield Promise.all([tab.REPL.qexec(`kubectl get taskrun -l tekton.dev/pipelineRun=${run.metadata.name} -o json`), tab.REPL.qexec(`kubectl get pods -n ${run.metadata.namespace} -l tekton.dev/pipelineRun=${run.metadata.name} -o json`)]);
    const containers = (0, _core.flatten)(pods.map(pod => {
      ...
      return pod.spec.containers.map((container, idx) => {
        const {
          containerID
        } = pod.status.containerStatuses[idx];
        ...
        return {
          ...
          attributes: [{
            ...
            key: 'STATUS',
            value: status,
            tag: 'badge' // css: cssForValue[status]
