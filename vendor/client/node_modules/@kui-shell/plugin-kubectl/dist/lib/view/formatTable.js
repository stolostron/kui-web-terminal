...
exports.stringToTable = exports.formatTable = exports.preprocessTable = void 0;
...
const formatTable = (command, verb, entityTypeFromCommandLine, args, preTable, nameColumn = 'NAME') => __awaiter(void 0, void 0, void 0, function* () {
  ...
  const rows = preTable.map((rows, idx) => {
    ...
    const css = '';
    const firstColumnCSS = idx === 0 || rows[0].key !== 'CURRENT' ? css : 'selected-entity'; // if we have a "name split", e.g. "pod/myPod", then keep track of the "pod" part
    ...
    const rowCSS = [rowIsSelected ? 'selected-row' : ''].filter(_ => _); // if there isn't a global namespace specifier, maybe there is a row namespace specifier
    ...
    const onclick = idx === 0 ? false : drilldownVerb ? `${drilldownCommand} ${drilldownVerb}${drilldownKind(nameSplit, rows)} ${(0, _core.encodeComponent)(nameForDrilldown)} ${drilldownFormat} ${ns}` : false;
    ...
    return {
      ...
      attributes: rows.slice(1).map(({
        ...
        outerCSS: header + ' ' + (outerCSSForKey[key] || '') + (colIdx <= 1 || colIdx === nameColumnIdx - 1 || columnVisibleWithSidecar.test(key) ? '' : ' hide-with-sidecar'),
        css: css + ' ' + (idx > 0 && cssForKey[key] || '') + ' ' + (_cssForValue.default[column] || key === 'READY' && cssForReadyCount(column) || maybeRed(column)),
