...
describe(`kubectl label handling ${process.env.MOCHA_RUN_TARGET || ''}`, function () {
    ...
    synonyms.forEach(kubectl => {
        ...
        const expectLabelInSidecar = (key, value) => {
            it(`should show label ${key}=${value} in the sidecar`, () => __awaiter(this, void 0, void 0, function* () {
                try {
                    yield CLI.command(`${kubectl} get pod nginx -o yaml ${inNamespace}`, this.app)
                    ...
                    yield this.app.client.waitForVisible(Selectors.SIDECAR_MODE_BUTTON_SELECTED('raw'));
                    yield SidecarExpect.yaml({ metadata: { labels: { [key]: value } } });
                }
                catch (err) {
