...
${isDeprecated ? `### Warnings\n${strings('This API Resource is deprecated')}` : ''}
    `
                        ...
                        .concat(requiredFields.length === 0 ? [] : [{ label: 'Required Fields', items: requiredFields.map(formatField) }])
                        .concat(notRequiredFields.length === 0 ? [] : [{ label: 'Fields', items: notRequiredFields.map(formatField) }])
                };
            }
        }
        catch (err) {
