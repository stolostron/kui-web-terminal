...
const parseIstio = (raw, execOptions) => {
    ...
    return raw
        ...
        try {
            ...
            const zapr = {
                timestamp: timestamp
                    ? core_1.prettyPrintTime(timestamp, timestampFormat, prevTimestamp, core_1.withLanguage(execOptions))
                    : '',
                rawTimestamp: timestamp,
                logType,
            ...
            const zapr = {
                timestamp: timestamp && core_1.prettyPrintTime(timestamp, timestampFormat, prevTimestamp, core_1.withLanguage(execOptions)),
                rawTimestamp: timestamp,
                logType,
                provider,
                origin,
...
exports.processLogs = (raw, execOptions, options = new AsHTML()) => {
    return parseZapr(raw) || parseIstio(raw, execOptions) || parseCloudLens(raw, execOptions, options);
};
function formatLogs(raw, execOptions) {
    return new AsHTML().format(exports.processLogs(raw, execOptions));
}
