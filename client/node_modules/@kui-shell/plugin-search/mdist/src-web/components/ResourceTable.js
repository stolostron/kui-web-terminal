...
var ResourceTable = /** @class */ (function (_super) {
    ...
    function ResourceTable(props) {
        ...
        _this.tabHeaders = ['name', 'action', 'share', 'edit'];
        ...
        _this.handleEvent = function (row, cell, e) {
            if ((e && e.which === 13) || !e) {
                var item = _this.props.items.filter(function (data) { return data['name'] === lodash.get(row, '[0].value', ''); });
                switch (cell.info['header']) {
                    case 'action':
                        _this.setState({ itemForAction: lodash.get(item, '[0]', ''), modalOpen: true, action: 'remove' });
                        break;
                    case 'edit':
                        _this.setState({ itemForAction: lodash.get(item, '[0]', ''), modalOpen: true, action: 'edit' });
                        break;
                    case 'share':
                        _this.setState({ itemForAction: lodash.get(item, '[0]', ''), modalOpen: true, action: 'share' });
                        break;
                    default:
                        var headers_1 = ['name', 'namespace', 'cluster']; // Headers used for filters keywords
                        var filters_1 = ''; // Query filters
                        // Since multiple resources can have the same name, get the namespace and cluster name for that resource
                        var _1 = row.filter(function (data) { return (data && data.value && (headers_1.includes(data.info['header']))); });
                        _1.forEach(function (data) {
                            filters_1 += data.info['header'] + ":" + data.value + " ";
                        });
                        if (_this.props.kind === 'savedSearches' && cell.info['header'] === 'name') {
                            ...
                            return core_1.getCurrentTab().REPL.pexec("search " + row[2].value);
                        }
                        else if (cell.info['header'] === 'name' && filters_1) {
                            return core_1.getCurrentTab().REPL.pexec("search summary kind:" + _this.props.kind + " " + filters_1);
                        }
                        else {
                            return null;
                        }
    ...
    ResourceTable.prototype.getRows = function () {
        ...
        return visibleItems.map(function (item, i) {
            ...
            if (_this.props.kind !== 'cluster' && _this.props.kind !== 'release') {
                row.action = (React.createElement(icons_react_1.Delete16, { className: 'table-action', onClick: function () { return _this.setState({ itemForAction: item, modalOpen: true, action: 'remove' }); } }));
                if (kind === 'savedSearches') {
                    row['share'] = (React.createElement(icons_react_1.Share16, { className: 'table-action', onClick: function () { return _this.setState({ itemForAction: item, modalOpen: true, action: 'share' }); } }));
                    row['edit'] = (React.createElement(icons_react_1.Edit16, { className: 'table-action', onClick: function () { return _this.setState({ itemForAction: item, modalOpen: true, action: 'edit' }); } }));
                }
            }
            return row;
        });
            ...
            !collapse
                ? React.createElement(React.Fragment, null,
                                        ...
                                        ? React.createElement("div", { onClick: _this.handleSort(header.key), className: "bx--table-sort-v2" + (sortDirection === 'asc' ? ' bx--table-sort-v2--ascending' : '') + (sortColumn === header.key ? ' bx--table-sort-v2--active' : ''), "data-key": header.key },
                                            React.createElement("span", { className: 'bx--table-header-label' },
                                                ...
                                                React.createElement("span", { className: 'arrow-header-label' }, _this.state.sortDirection === 'asc'
                                                    ...
                                                    : React.createElement("span", null, "\u25BC"))))
                                        : null)); }))),
                                React.createElement(TableBody, null, rows.map(function (row) { return (React.createElement(TableRow, { key: row.id, className: 'bx--data-table--compact' }, row.cells.map(function (cell) { return React.createElement(TableCell, { onKeyPress: function (e) { return _this.handleEvent(row.cells, cell, e); }, tabIndex: _this.tabHeaders.includes(cell.info['header']) ? 0 : null, key: cell.id, onClick: function () { return _this.handleEvent(row.cells, cell); } }, cell.info['header'] !== 'status'
                                    ? cell.value
                                    : React.createElement("div", null,
                                        status_1.getStatusIcon(cell.value),
                    ...
                    this.props.items.length > PAGE_SIZES.DEFAULT
                        ...
                        : null)
                : null,
            React.createElement(Modal_1["default"], { item: this.state.itemForAction, modalOpen: modalOpen, onClose: function () { return _this.setState({ itemForAction: {}, modalOpen: false, action: '' }); }, action: this.state.action })));
    };
