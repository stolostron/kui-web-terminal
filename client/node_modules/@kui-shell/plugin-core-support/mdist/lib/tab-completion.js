...
import { inBrowser, injectCSS, getCurrentBlock, getCurrentPrompt, getTabFromTarget, KeyCodes, split, _split, expandHomeDir, findFile, doCancel, isUsingCustomPrompt } from '@kui-shell/core';
                ...
                if (value) {
                    ...
                    const handleUsage = (usageError /* Errors.UsageError */) => {
                        ...
                        if (usage.fn) {
                            ...
                            if (positionals.length > 0) {
                                ...
                                if (commandIdx === args.length - 1 && !prompt.value.match(/\s+$/)) {
                                    ...
                                    try {
                                        ...
                                        suggest(param, findFile(args[commandIdx + lastIdx + 1], { safe: true }), block, prompt, temporaryContainer, commandIdx + lastIdx);
                                    }
                                    catch (err) {
                                        console.error(err);
                                    }
                            ...
                            const { A: args, endIndices } = _split(prompt.value, true, true);
