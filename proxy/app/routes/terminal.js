...
//initUserSession will login user with cloudctl and bind websocket with a kui subprocess
const initUserSession = async (ws, accessToken, locale) => {
    let user = {};
    try {
        user = await getUser(accessToken);
        const child = createChild(user, locale);
        const channel = CustomStdioChannelWebsocketSide(ws);
        ...
        if (pathname === wsURL) {
            try {
                ...
                wss.handleUpgrade(request, socket, head, function done(ws) {
                    ...
                    //set up child process and 
                    initUserSession(ws, accessToken, locale).catch(e => {
