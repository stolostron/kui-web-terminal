...
describe('bash-like host catchall', function () {
    before(Common.before(this, { noApp: isHeadless }));
    if (!isHeadless) {
        after(Common.after(this));
    }
    if (isHeadless) {
        it('should show some output for host google.com in headless mode', () => headlessCLI
            .command('host google.com')
            .then(headlessCLI.expectOK('has address'))
            .catch(Common.oops(this)));
    }
    else {
        it('should show some output for host google.com in non-headless mode', () => headfullCLI
            .command('host google.com', this.app)
            .then(ReplExpect.okWithString('has address'))
            .catch(Common.oops(this)));
    }
});
//# sourceMappingURL=headless-host.js.map