...
import { isMetadataBearing, isMetadataBearingByReference } from '../models/entity';
import { isButton } from '../models/mmr/types';
...
export function isSidecarMode(entity) {
    const mode = entity;
...
export const css = {
    buttons: (tab) => sidecarSelector(tab, rawCSS.buttons),
    backContainer: (tab) => sidecarSelector(tab, '.sidecar-bottom-stripe .sidecar-bottom-stripe-left-bits .sidecar-bottom-stripe-back-bits'),
    backButton: (tab) => sidecarSelector(tab, '.sidecar-bottom-stripe .sidecar-bottom-stripe-left-bits .sidecar-bottom-stripe-back-button'),
    ...
    modeContainer: (tab) => sidecarSelector(tab, '.sidecar-top-stripe .sidecar-bottom-stripe-left-bits .sidecar-bottom-stripe-mode-bits .bx--tabs__nav'),
    bottomContainer: (tab) => sidecarSelector(tab, '.sidecar-bottom-stripe-toolbar .sidecar-bottom-stripe-mode-bits'),
    ...
    button.onclick = () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const present = (view) => __awaiter(void 0, void 0, void 0, function* () {
            if (typeof view === 'string') {
                ...
                dom.innerText = view;
                const { insertCustomContent } = yield import('./views/sidecar');
        ...
        if (isButton(opts)) {
            buttonOnclick(tab, entity, opts);
        }
        else if (hasContent(opts)) {
            const { formatForTab } = yield import('../models/mmr/show');
...
export const addModeButton = (tab, mode, 
...
export const addModeButtons = (tab, modesUnsorted = [], entity, options) => {
